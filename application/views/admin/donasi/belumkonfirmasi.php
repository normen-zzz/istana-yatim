									...
									<div class="table-responsive">
										<table class="table table-bordered table-md" id="myTable">
											...
											<tbody>
												...
												<tr>
													<td><?= $d['id_donasi'] ?></td>
													<td><?= strftime("%A %d-%B-%Y %T", strtotime($d['tanggal'])) ?></td>
													<td><?= $d['nama'] ?></td>
													<td><?= $d['nowa'] ?></td>
													<td><?= rupiah($d['jumlah']) ?></td>
													<td><?= $d['bank'] ?></td>
													<td><a href=""
															onclick="window.open('<?= base_url('assets/images/donasi/') . $d['bukti'] ?>','targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=1090px, height=550px, top=25px left=120px'); return false;"><img
																style="width: 200px" src="<?= base_url('assets/images/donasi/') . $d['bukti'] ?>"></a>
													</td>
													<!-- <td><img style="width: 200px" src="<?= base_url('assets/images/donasi/') . $d['bukti'] ?>"></td> -->
													<td> <a href="<?= base_url('admin/donasi/konfirmasi/'). $d['id_donasi'] ?>"
															class="btn btn-success"
															onclick="return confirm('kamu yakin akan Konfirmasi Donasi  ?');">Konfirmasi</a> <a
															href="<?= base_url('admin/donasi/deletedonasibelumkonfirmasi/') . $d['id_donasi'] ?>"
															class="btn btn-danger" onclick="return confirm('kamu yakin akan menghapus  ?');">Hapus</a>
													</td>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<?php echo form_open_multipart('admin/cms/tambahdonasiAct'); ?>
			...
			function confir() {
				swal({
					...
					.then((willDelete) => {
						if (willDelete) {
							swal({
								title: "Hapus Donasi Berhasil",
								icon: "success"
							}).then(okay => {
								if (okay) {
									window.location.href = "<?= base_url('admin/donasi/deletedonasi/') . $d['id_donasi'] ?>";
								}
							});
